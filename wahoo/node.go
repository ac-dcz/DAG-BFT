...
type Node struct {
	...
	ready         map[uint64]map[string]map[string][]byte // map from round to block_sender to ready_sender to parSig
...
func NewNode(conf *config.Config) *Node {
	...
	n.ready = make(map[uint64]map[string]map[string][]byte)
	n.round = 1
	...
	n.blockSend = make(map[uint64]bool)
...
func (n *Node) RunLoop() {
	...
	n.logger.Info("the number of qurey", "number", n.blockQurey)
}
...
func (n *Node) storeReady(ready *Ready) {
	if _, ok := n.ready[ready.Round]; !ok {
		n.ready[ready.Round] = make(map[string]map[string][]byte)
	}
	if _, ok := n.ready[ready.Round][ready.BlockSender]; !ok {
		n.ready[ready.Round][ready.BlockSender] = make(map[string][]byte)
	}
	n.ready[ready.Round][ready.BlockSender][ready.ReadySender] = ready.PartialSig
}
...
func (n *Node) tryToElectLeader(round uint64) {
	...
	if len(elect) >= n.quorumNum && !n.leaderElect[round] {
		...
		n.broadcastReVote(round-1, true) // TO DO...
